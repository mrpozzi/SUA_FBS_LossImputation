...
*~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~*/ 
                    ...
                    ///////////////// BUILD CATEGORIES & IMPORT SUA WORKING DATA  //////////////////// 
...
// IMPORT SUA WORKING SYSTEM DATA AND CATEGORIZE COMMODITIES 
					 
//insheet using "$foldcsv\SUA_waste.csv",clear 
// Modification by GARIERI
insheet using "$foldcsv/SUA_waste.csv",clear 
...
replace areaname ="Côte d'Ivoire" if areaname=="CÃ´te d'Ivoire" 
replace areaname ="Réunion" if areaname=="RÃ©union" 
...
drop num_2013-symb_2015 
save "sua data_waiting for elaboration.dta",replace 
 
						/////////////////  SPLIT AND RESHAPE SUA WORKING DATA  /////////////////// 
		 
use "sua data_waiting for elaboration.dta",clear 
 
levelsof foodgroup, local(allcategories)  
foreach c of local allcategories { 
use "sua data_waiting for elaboration.dta",clear 
 
keep if foodgroup==`c' 
 
forvalues i=1961(1)2012 { 
replace symb_`i'="_" if symb_`i'=="" 
} 
 
reshape long num_ symb_ , i(itemcode itemname areacode areaname elementcode) j(year) 
cap drop elementname 
reshape wide num_ symb_ , i(itemcode itemname areacode areaname year) j(elementcode) 
  
save "sua data reshaped_`c'.dta",replace 
 
} 
                               /////////////////  APPEND SPLITTED DATASETS  /////////////////// 
use "sua data_waiting for elaboration.dta",clear 
...
set obs 0 
foreach c of local allcategories { 
append using "sua data reshaped_`c'.dta" 
*Drop observations with no Production and no Loss. (Can be kept on stronger computers.) 
drop if (num_51==0 | num_51==.) & (num_121~=0 | num_121~=.) 
} 
 
 
save "SUA_waste reshaped.dta",replace 
 
foreach c of local allcategories { 
erase "sua data reshaped_`c'.dta" 
} 
